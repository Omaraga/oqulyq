...
$model->loadAnswers();
$this->registerJsFile('/js/tasks/sudoku-img-view.js',['depends' => [\yii\web\JqueryAsset::className()]]);


?>
<style>
    #sudoku-table tr, table, td{
        padding: 10px;
...
<input type="hidden" id="answer" value="<?=htmlspecialchars($model->answers->name);?>">
<input type="hidden" id="answerUrl" value="<?=htmlspecialchars($model->answers->url);?>" data-url="<?=Yii::$app->params['url'];?>">
